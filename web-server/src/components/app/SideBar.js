...
export default function SideBar({ className }) {
  ...
  const { user, isGuestOrFreeUser, isGuestUser, isFreeUser } =
  ...
  const NavLink = ({
    ...
    if (!override && disable) {
      onClick = (e) => {
        e.preventDefault();
        setShowUnlockFeatureModal(true);
      };
    }
    return (
      <Link
        ...
        className={
          "w-100 p-2 px-3 d-inline-block link-underline link-underline-opacity-0 " +
          activeClass +
          className
        }
        to={_to}
  ...
  return (
    <div
      className={
        "d-flex flex-column flex-shrink-0 text-bg-dark shadow bg-body border-end " +
        className
      }
    ...
    >
      <Link
        ...
        reloadDocument={true}
        style={{ height: "60px" }}
        className="d-flex align-items-center m-3 my-2 text-white text-decoration-none"
      >
      ...
      <div className="px-4 mb-3">
        <NavLink
          to="/app/quick-share"
          className={"btn text-start rounded-pill border border-secondary"}
          override={true}
        >
      ...
      {!isSelfHosted() && (
        <div>
          ...
          <ul className="nav nav-pills flex-column px-2">
            <li>
              <NavLink
                to={isGuestUser() ? "/signup" : "/app/account"}
                reloadDocument={isGuestUser()}
                override={true}
              >
      ...
      <div className="d-flex justify-content-between p-3">
        <small className="text-body-secondary">&copy; 2024 Daan Vervacke</small>
        <div>
          <a
            className="text-body-secondary"
            href="https://github.com/DaanVervacke/transfer.zip-web"
          >
